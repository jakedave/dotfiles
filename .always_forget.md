...
- `brew deps --tree --installed`                                                               = list packages and dependencies as a tree
- `brew leaves --installed-on-request`                                                         = list packages explicitly installed by user
- `brew info <package>`                                                                        = describe package

## rvm
- `rvm reset`                                                                                  = resolve 'Warning! PATH is not properly set up'
...
- `aws ecr delete-repository --repository-name old-app --force`                                = delete ECR
- `aws ssm get-parameter --name <name>`                                                        = get parameter with <name>
- `aws eks list-clusters --output text`                                                        = list available eks clusters (within current AWS account)
- `aws eks update-kubeconfig --name <name> --alias <name>`                                     = configure cluster as available kube context
- `aws logs describe-log-groups --log-group-name-prefix <prefix>`                              = describe cloudwatch log group with prefix <prefix>
- `aws logs tag-resource --resource-arn <arn> --tags <key>=<value>`                            = tag loggroup with <tags> on <arn>

## eksctl
- `eksctl get nodegroup --cluster <cluster>`                                                   = get nodegroup by kubecontext <cluster>
- `eksctl scale nodegroup --name=dev --cluster=dev --nodes=1 --nodes-min=1 --nodes-max=1`      = scale nodegroup max,min,desired nodes
...
## Flux
- flux logs --follow                                                                           = tail flux reconciliation logs

## Terraform
- `terraform force-unlock <id>`                                                                = unlock state file with <id>
